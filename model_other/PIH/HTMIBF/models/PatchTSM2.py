...
from layers.PatchTSM_backbone2 import PatchTSM_backbone2
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
                 ...
                 pretrain_head: bool = False, head_type='flatten', verbose: bool = False,seperate_mlp=False,mlp_dropout=0.3, **kwargs):
        ...
        if self.decomposition:
            ...
            self.model_trend = PatchTSM_backbone2(c_in=c_in, context_window=context_window, target_window=target_window,
            ...
            self.model_res = PatchTSM_backbone2(c_in=c_in, context_window=context_window, target_window=target_window,
                                                patch_len=patch_len, stride=stride,
                                                max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                                n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                                attn_dropout=attn_dropout,
            ...
            self.model = PatchTSM_backbone2(c_in=c_in, context_window=context_window, target_window=target_window,
                                            patch_len=patch_len, stride=stride,
                                            max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                            n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                            attn_dropout=attn_dropout,
                                            ...
                                            pretrain_head=pretrain_head, head_type=head_type, individual=individual,
                                            revin=revin, affine=affine,
                                            subtract_last=subtract_last, verbose=verbose, share=configs.share, type=configs.type,
                                            t_layers=configs.t_layers, tran_first=configs.tran_first, K=configs.K,temp=configs.temp,IB=configs.IB,seperate_mlp=configs.seperate_mlp,mlp_dropout=configs.mlp_dropout)
    ...
    def forward(self, x):  # x: [Batch, Input length, Channel]
        if self.decomposition:
            ...
            res,KL_loss_res= self.model_res(res_init)
            trend,KL_loss_trend = self.model_trend(trend_init)
            KL_loss = KL_loss_res + KL_loss_trend
            x = res + trend
            x = x.permute(0, 2, 1)  # x: [Batch, Input length, Channel]
        else:
            x = x.permute(0, 2, 1)  # x: [Batch, Channel, Input length]
            x,KL_loss = self.model(x)
            x = x.permute(0, 2, 1)  # x: [Batch, Input length, Channel]
        return x,KL_loss
