"""PatchTST with TimeRole's compressed-history correction mechanism."""
...
from models.PatchTSTRecent import Model as RecentPatchTST


class Model(RecentPatchTST):
    def __init__(self, configs):
        ...
        self.memory_pool = int(
            getattr(
                ...
                getattr(configs, "cmrhm_memory_pool", 16),
            )
        )
        self.old_len = self.input_seq_len - self.recent_len
        if self.old_len <= 0 or self.old_len % self.memory_pool:
            raise ValueError("TimeRole old history must be positive and divisible by pool")
        ...
        self.memory_scale = nn.Parameter(torch.zeros(int(configs.enc_in)))
        torch.set_rng_state(cpu_rng_state)

    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        base_output = super().forecast(x_enc, x_mark_enc, x_dec, x_mark_dec)
        ...
        delta = (with_memory - without_memory).permute(0, 2, 1)
        scale = torch.tanh(self.memory_scale)[None, None, :]
        return base_output + scale * delta * stdev
