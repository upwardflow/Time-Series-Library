"""TimeRole: role-differentiated historical modeling for forecasting."""
...
class Model(RecentGraphMamba):
    ...
    def __init__(self, configs):
        ...
        self.old_history_intervention = getattr(
            ...
            getattr(configs, "cmrhm_old_intervention", "intact"),
        )
        self.memory_noise_std = float(
            getattr(
                configs,
                "timerole_noise_std",
                getattr(configs, "cmrhm_noise_std", 1.0),
            )
        )
        if self.old_history_intervention not in {
            ...
            raise ValueError(
                f"Unsupported TimeRole old-history intervention: "
