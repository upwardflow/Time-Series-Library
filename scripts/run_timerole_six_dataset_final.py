#!/usr/bin/env python3
"""Overnight final-test matrix for the frozen TimeRole-v1 model.
...
EXISTING_ETTM = ROOT / "logs" / "timerole_final_test" / "records"
...
def build_command(dataset: str, pred_len: int, args: argparse.Namespace) -> list[str]:
    ...
    return [
        ...
        # Required when TimeRole's recent-96 backbone resolves hourly
...
def existing_ettm_record(dataset: str, pred_len: int) -> dict[str, object] | None:
    source = EXISTING_ETTM / f"{dataset.lower()}_{pred_len}_timerole_s{SEED}.json"
...
def write_outputs(rows: list[dict[str, object]], current: str) -> None:
    ...
    lines = [
        ...
        f"- 当前任务：`{current}`",
        "- 模型：冻结的 `TimeRole-v1`，输入长度 336，主干最近窗口 96",
    ...
    lines.extend([
        ...
        "ETTh1/ETTh2 使用周期双尺度近期主干；ETTm1/ETTm2、Weather、Solar 使用独立双 Patch 近期主干。两类配置均属于 TimeRole。",
        "",
