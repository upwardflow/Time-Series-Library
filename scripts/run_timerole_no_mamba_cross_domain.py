#!/usr/bin/env python3
"""Run validation-only cross-domain TimeRole ablations without Mamba.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "logs" / "timerole_no_mamba_cross_domain",
    )
    parser.add_argument(
...
def build_train_command(
    ...
    return [
        ...
        "--model", "TimeRole", "--seed", str(args.seed),
...
def run_one(dataset: str, horizon: int, args: argparse.Namespace) -> int:
    ...
    if not completed(train_path, "best_mse"):
        ...
        payload: dict[str, object] = {
            ...
            "stage": "training", "model": "TimeRole",
            "dataset": dataset, "horizon": horizon,
            "variant": "no_mamba", "variant_label": "TimeRole + w/o Mamba",
    ...
    payload = {
        "status": "completed" if code == 0 and metrics else "failed",
        "stage": "checkpoint_validation", "model": "TimeRole",
        "dataset": dataset, "horizon": horizon,
        "variant": "no_mamba", "variant_label": "TimeRole + w/o Mamba",
...
def summarize(args: argparse.Namespace) -> int:
    ...
    for dataset in args.datasets:
        for horizon in args.horizons:
            ...
            rows.append({
                "model": "TimeRole", "variant": "no_mamba",
                "variant_label": "TimeRole + w/o Mamba",
...
def main() -> int:
    ...
    for dataset in args.datasets:
        for horizon in args.horizons:
            print(f"=== TimeRole + w/o Mamba | {dataset}-{horizon} ===", flush=True)
