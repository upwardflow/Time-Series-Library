#!/usr/bin/env python3
"""Validation-only efficiency benchmark for Recent96, Raw336, and TimeRole."""
...
OUTPUT = ROOT / "logs" / "timerole_efficiency"
DATASETS = ("ETTm1", "ETTm2")
HORIZONS = (96, 720)
VARIANTS = ("Recent96", "Raw336", "TimeRole")
...
def source(dataset: str, horizon: int, variant: str) -> Path:
    if variant == "Raw336":
        return (
            ROOT / "logs" / "timerole_strict_evidence" / "validation"
            / f"b_{dataset.lower()}_p{horizon}_raw_s2021.json"
        )
    label = "recent336" if variant == "Recent96" else "timerole"
    return (
        ROOT / "logs" / "timerole_all_horizons" / "validation"
...
def run_one(dataset: str, horizon: int, variant: str, args: argparse.Namespace) -> int:
    ...
    if variant == "TimeRole":
        replace(command, "--timerole_old_intervention", "intact")
    if args.dry_run:
        print(" ".join(command))
        return 0
    env = os.environ.copy()
