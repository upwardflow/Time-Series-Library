#!/usr/bin/env python3
"""Build the unified non-periodic CMRHM main-result set without Traffic.
...
OUTPUT = ROOT / "logs" / "cmrhm_unified_main"
...
def destination(dataset: str, horizon: int) -> Path:
    return OUTPUT / "records" / f"cmrhm_{dataset.lower()}_p{horizon}_s2021.json"
...
def reuse_source(dataset: str, horizon: int) -> Path | None:
    ...
    if dataset in {"weather", "solar"}:
        return (
            ROOT / "logs" / "graphmamba_cmrhm_six_dataset_final" / "records"
...
def test_existing_checkpoint(
    ...
    log_path = OUTPUT / "logs" / f"cmrhm_{dataset.lower()}_p{horizon}_s2021.log"
    if args.dry_run:
...
def train_missing(dataset: str, horizon: int, args: argparse.Namespace) -> int:
    candidate = f"cmrhm_unified_{dataset.lower()}_p{horizon}_s2021"
    command = [
        sys.executable, "-u", str(ROOT / "scripts" / "run_graphmamba_innovation.py"),
        "--model", "GraphMambaCMRHM", "--candidate", candidate,
    ...
    payload = {
        **trained,
        "model": "CMRHM",
        "implementation_model": "GraphMambaCMRHM",
        "host_backbone": "graph_enhanced_ssm",
        "seq_len": 336,
        "recent_len": 96,
        "scan_mode": "independent_shared",
...
def write_manifest_and_table() -> tuple[int, int]:
    ...
    manifest = {
        "protocol": "CMRHM unified independent_shared main results",
        "datasets": DATASETS,
        "horizons": HORIZONS,
