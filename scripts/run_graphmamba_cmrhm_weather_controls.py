#!/usr/bin/env python3
"""Run recent-only Weather controls for the frozen CMRHM domain audit.

Only horizons 96 and 720 are used as short/long endpoint checks.  The paired
CMRHM validation metrics already exist from the frozen six-dataset run.  This
...
RUN_PY = ROOT / "run.py"
OUTPUT = ROOT / "logs" / "graphmamba_cmrhm_weather_controls"
HORIZONS = (96, 720)
...
def finalize() -> None:
    full_source = ROOT / "logs" / "graphmamba_cmrhm_six_dataset_final" / "records"
    rows = []
    for horizon in HORIZONS:
        ...
        cmrhm_mse = float(full["validation_best_mse"])
        cmrhm_mae = float(full["validation_best_mae"])
        rows.append(
            {
                "horizon": horizon,
                "recent_mse": recent_mse,
                "cmrhm_mse": cmrhm_mse,
                "mse_improvement_pct": 100 * (recent_mse - cmrhm_mse) / recent_mse,
                "recent_mae": recent_mae,
                "cmrhm_mae": cmrhm_mae,
                "mae_improvement_pct": 100 * (recent_mae - cmrhm_mae) / recent_mae,
            }
        )
