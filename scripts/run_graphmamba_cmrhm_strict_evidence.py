#!/usr/bin/env python3
"""Run the preregistered 52-job, validation-only CMRHM-v1 audit."""
...
OUTPUT = ROOT / "logs" / "graphmamba_cmrhm_strict_evidence"
...
def jobs() -> list[dict]:
    ...
    # Group B: Recent336 and CMRHM336 are reused; only train Raw336.
    for dataset in DATASETS:
        for horizon in HORIZONS:
            result.append(dict(group="b", dataset=dataset, horizon=horizon,
                               seed=2021, variant="raw", model="GraphMamba"))
    # Group C: full CMRHM is reused; train three isolated controls.
    controls = (("cat", "GraphMambaCMRHMConcat"),
                ("nd", "GraphMambaCMRHMNoDiff"),
                ("gg", "GraphMambaCMRHMGlobalGate"))
    for dataset in DATASETS:
...
def write_status(done: list[str], failed: list[str], active: str | None) -> None:
    ...
    temporary.write_text(json.dumps({
        "protocol": "CMRHM-v1 strict evidence audit; validation only",
        "total_new_jobs": 52,
