#!/usr/bin/env python3
"""Aggregate the frozen CMRHM-v1 validation audit and apply its fixed gates."""
...
NEW = ROOT / "logs" / "graphmamba_cmrhm_strict_evidence" / "validation"
OLD = ROOT / "logs" / "graphmamba_cmrhm_all_horizons" / "validation"
OUT = NEW.parent
REPORT = ROOT / "experiment_results" / "GraphMamba_CMRHM_strict_evidence_result.md"
    ...
    for dataset in DATASETS:
        for horizon in HORIZONS:
            ...
            for seed in SEEDS:
                ...
                full = old(dataset, horizon, "cmrhm") if seed == 2021 else new("a", dataset, horizon, "c", seed)
                if recent and full:
                    row = {
                        "dataset": dataset, "horizon": horizon, "seed": seed,
                        "recent_mse": recent["best_mse"], "cmrhm_mse": full["best_mse"],
                        "mse_improvement_pct": improve(recent["best_mse"], full["best_mse"]),
                        "recent_mae": recent["best_mae"], "cmrhm_mae": full["best_mae"],
            ...
            if len(task) == 3:
                summaries.append({
                    ...
                    "cmrhm_mse_mean": avg(x["cmrhm_mse"] for x in task),
                    "cmrhm_mse_sd": statistics.stdev(x["cmrhm_mse"] for x in task),
                    "cmrhm_mean_mse_improvement_pct": improve(
                        avg(x["recent_mse"] for x in task), avg(x["cmrhm_mse"] for x in task)),
                    "recent_mae_mean": avg(x["recent_mae"] for x in task),
                    "recent_mae_sd": statistics.stdev(x["recent_mae"] for x in task),
                    "cmrhm_mae_mean": avg(x["cmrhm_mae"] for x in task),
                    "cmrhm_mae_sd": statistics.stdev(x["cmrhm_mae"] for x in task),
                })
...
def group_b() -> list[dict]:
    ...
    for dataset in DATASETS:
        for horizon in HORIZONS:
            recent, raw, full = old(dataset, horizon, "recent336"), new("b", dataset, horizon, "raw", 2021), old(dataset, horizon, "cmrhm")
            if recent and raw and full:
                rows.append({
                    "dataset": dataset, "horizon": horizon,
                    "recent_mse": recent["best_mse"], "raw_mse": raw["best_mse"], "cmrhm_mse": full["best_mse"],
                    "cmrhm_vs_raw_mse_pct": improve(raw["best_mse"], full["best_mse"]),
                    "recent_mae": recent["best_mae"], "raw_mae": raw["best_mae"], "cmrhm_mae": full["best_mae"],
                    "cmrhm_vs_raw_mae_pct": improve(raw["best_mae"], full["best_mae"]),
                    "recent_params": PARAMS[horizon][0], "raw_params": PARAMS[horizon][1], "cmrhm_params": PARAMS[horizon][2],
                    "raw_runtime_s": raw["duration_seconds"], "cmrhm_runtime_s": full["duration_seconds"],
                })
    return rows
...
def group_c() -> list[dict]:
    ...
    for dataset in DATASETS:
        for horizon in (96, 720):
            full = old(dataset, horizon, "cmrhm")
...
def main() -> int:
    ...
    a_mse_wins = sum(x["cmrhm_mse"] < x["recent_mse"] for x in a)
    a_mae_wins = sum(x["cmrhm_mae"] < x["recent_mae"] for x in a)
    ...
    b_wins = sum(x["cmrhm_mse"] < x["raw_mse"] for x in b)
    b_mse = avg(x["cmrhm_vs_raw_mse_pct"] for x in b); b_mae = avg(x["cmrhm_vs_raw_mae_pct"] for x in b)
    ...
    lines = ["# GraphMambaCMRHM-v1 严格有效性审计", "", f"更新时间：{datetime.now().astimezone().isoformat(timespec='seconds')}", "",
             ...
             f"- 平均训练时间：Raw336 {avg(x['raw_runtime_s'] for x in b):.1f}s，CMRHM {avg(x['cmrhm_runtime_s'] for x in b):.1f}s。", "",
