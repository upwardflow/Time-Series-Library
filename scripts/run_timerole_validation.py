#!/usr/bin/env python3
"""Strict paired validation gate for TimeRole on ETTm1/ETTm2-720."""
...
def main() -> int:
    ...
    for dataset, pred_len in TASKS:
        ...
        baseline, candidate = records["recent336"], records["timerole"]
        rows.append({
            "dataset": dataset, "pred_len": pred_len, "seed": 2021,
            "baseline_mse": baseline["best_mse"], "timerole_mse": candidate["best_mse"],
            "mse_improvement_pct": 100 * (baseline["best_mse"] - candidate["best_mse"]) / baseline["best_mse"],
            "baseline_mae": baseline["best_mae"], "timerole_mae": candidate["best_mae"],
            "mae_improvement_pct": 100 * (baseline["best_mae"] - candidate["best_mae"]) / baseline["best_mae"],
            "baseline_best_epoch": baseline["best_epoch"], "timerole_best_epoch": candidate["best_epoch"],
        })
