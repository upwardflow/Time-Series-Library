#!/usr/bin/env python3
"""Evaluate frozen Recent336/TimeRole validation checkpoints on the test split once."""
...
VALIDATION = ROOT / "logs" / "timerole_all_horizons" / "validation"
OUTPUT = ROOT / "logs" / "timerole_final_test"
...
def collect() -> list[dict[str, object]]:
    ...
    for dataset in DATASETS:
        for pred_len in PRED_LENS:
            ...
            if len(pair) != 2:
                continue
            baseline, timerole = pair["recent336"], pair["timerole"]
            rows.append({
                "dataset": dataset,
                "pred_len": pred_len,
                "seed": SEED,
                "baseline_test_mse": baseline["test_mse"],
                "timerole_test_mse": timerole["test_mse"],
                "mse_improvement_pct": 100 * (baseline["test_mse"] - timerole["test_mse"]) / baseline["test_mse"],
                "baseline_test_mae": baseline["test_mae"],
                "timerole_test_mae": timerole["test_mae"],
                "mae_improvement_pct": 100 * (baseline["test_mae"] - timerole["test_mae"]) / baseline["test_mae"],
            })
    return rows
