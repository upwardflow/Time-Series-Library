...
扩大回溯窗口能够为长期时间序列预测提供更多历史信息，但也会引入冗余观测、状态失配和额外计算。现有方法通常以统一编码流程处理全部历史，尚未充分建模近期状态与更早观测在预测中的异质作用。本文提出 TimeRole，一种近期—远期职责分化预测模型。其高分辨率近期路径采用图增强状态空间预测器联合编码当前时间动态和变量关系，并生成基础预测；低分辨率远期路径则将更早历史压缩为条件修正表示。压缩历史修正分支通过共享解码器比较加入历史前后的输出，并以有界通道系数将边际预测变化注入基础预测。在五个数据集、四个预测跨度的统一三随机种子比较中，TimeRole 分别取得12个最佳 MSE 和11个最佳 MAE，两项指标的平均排名为1.85和1.65。在 ETTm1 和 ETTm2 的配对对照中，TimeRole 相对近期预测器取得24/24个种子级 MSE 与 MAE 胜场。成本分析进一步表明，TimeRole 以接近近期预测器的资源开销利用更长历史，并大幅降低了直接长输入所需的参数量、推理延迟和显存。这些结果表明，按时间范围分配建模职责是兼顾长历史利用与计算效率的有效路径。
...
主比较包含 DLinear、PatchTST、iTransformer、TimeMixer、TimesNet、S-Mamba 和 MSGNet，分别代表线性、patch Transformer、倒置 Transformer、多尺度混合、二维变化建模、双向状态空间建模与多尺度变量图建模路线 [@Wang2025SMamba; @Cai2024MSGNet]。其中，S-Mamba 和 MSGNet 分别构成与 TimeRole 状态空间主干和变量关系建模最接近的外部基线。表2的主比较以及表3—4的核心消融均使用相同的数据划分、输入长度、评价脚本、验证集选点规则和随机种子集合 $\{2021,2022,2023\}$，并报告测试 MSE 与 MAE 的三种子均值和样本标准差。表5—7的历史干预、迁移实验和资源测量使用 seed 2021 的检查点。TimeMixer 在 ETTm2 和 Weather 上采用经验证集确定的学习率 $10^{-4}$，其余数据集使用 $10^{-2}$。其 ETTm1-96 和 ETTm1-720 的 seed 2023 运行出现数值发散，并在主表中如实标记。
...
表2报告五个数据集上的三随机种子测试 MSE 和 MAE。TimeRole 在20个数据集—跨度组合中取得12个最佳 MSE 和11个最佳 MAE，并分别另获3个与7个次优结果。其 MSE 和 MAE 的任务级平均排名分别为1.85和1.65，显示出稳定的整体竞争力。TimeRole 在 Weather 的全部四个跨度、ETTh2 的96—336跨度以及 ETTm1 的前三个跨度上表现尤为突出，并在其余多数任务上保持前列排名。与每个任务的最佳有效基线相比，TimeRole 的20任务宏平均 MSE 仅高0.234%，MAE 低0.059%。结合最优结果数量与平均排名，这些结果表明 TimeRole 在不同数据集和预测跨度上具有良好的综合预测性能。
...
为隔离远期历史修正的增益，表3在 ETTm1 和 ETTm2 上比较仅使用最后96点的 Recent96 与使用完整336点输入的 TimeRole。两者共享近期预测器和训练协议；TimeRole 将前240点输入压缩历史修正分支。每个检查点依据验证集 MSE 选择，表中报告三个随机种子的测试 MSE 均值与样本标准差。TimeRole 在8个任务上均降低了平均测试 MSE，任务级改善为6.884%–12.824%。展开到种子级后，测试 MSE 与 MAE 均取得24/24个配对胜场，配对宏平均改善分别为8.828%和4.162%。这些结果表明，压缩历史修正能够稳定增强近期预测器对长历史的利用。
...
表4进一步以三个随机种子比较完整 TimeRole 与三种简化实现：Concat 直接拼接近期和历史状态，NoDiff 移除配对输出差分，GlobalGate 以全局系数替代通道级系数。TimeRole 相对 Recent96 的测试 MSE 与 MAE 均取得12/12个种子级胜场。相对 Concat、NoDiff 和 GlobalGate，其 MSE 胜场分别为9/12、9/12和7/12，配对宏平均改善分别为0.743%、0.912%和0.189%。TimeRole 在两个 ETTm1 任务上同时取得最低 MSE 和 MAE，并在 ETTm2-720 上取得最低 MAE。综合结果支持以压缩远期历史形成条件修正的整体设计，同时显示具体条件化形式可以进一步根据数据特性调整。
...
为检验模型对远期内容的使用，我们固定已选择的检查点，在验证时对前240点历史执行样本错配、时间打乱、时间反转、近期均值替代和匹配尺度噪声替代。表5中的20个破坏性干预均使 MSE 上升，平均增幅为20.992%。其中，样本错配在全部任务上造成24.05%–75.91%的性能下降，表明 TimeRole 的预测确实依赖与当前样本匹配的远期历史。ETTm2 对时间打乱和反转的敏感度较低，进一步表明不同数据集对远期时序顺序的依赖程度存在差异。
...
为评估近期—远期职责分解的跨主干迁移性，我们将相同的远期条件修正路径接入非图结构 TimeXer。两种设置读取相同的336点样本，TimeXer 主干均处理最后96点，区别在于是否使用前240点的压缩历史修正。表6显示，加入该分支后，四个测试任务的 MSE 和 MAE 均获得改善，宏平均改善分别为5.673%和2.123%。这一结果支持将历史职责分化作为可接入不同近期预测主干的建模机制。
...
表7在相同硬件、批量大小与检查点评估流程下，对比 Recent96、直接处理336点历史的 Raw336 与 TimeRole。数值分别在 ETTm1 和 ETTm2 上取平均。相对 Recent96，TimeRole 的参数量仅增加约0.5%，每批推理延迟增加约9.1%，同时将四个任务的宏平均 MSE 降低5.215%。相对 Raw336，TimeRole 平均减少69.968%的参数量、43.505%的推理延迟和67.735%的峰值显存，四任务宏平均 MSE 相差2.046%。这一结果表明，TimeRole 能够以接近短输入模型的资源预算获取明确的长历史收益，并提供一条资源高效的长历史建模路径。
...
实验结果从总体性能、稳定增益、历史使用、跨主干迁移和资源开销五个方面支持近期—远期职责分化。TimeRole 的主要价值在于，它不再将长历史等同于更长的主干输入，而是根据时间距离为历史信息分配不同建模精度和预测职责。这种结构尤其适合需要扩展历史感受野、同时受制于计算成本的预测场景。当前实现采用固定的近期—远期划分、池化率与跨样本共享通道系数，不同数据域中的最优条件化形式也存在差异。因此，自适应历史范围、分辨率分配和样本条件化修正强度是进一步提升 TimeRole 的重要方向。

## 5 结论

本文提出 TimeRole，通过图增强状态空间近期预测器生成高分辨率基础预测，并通过压缩历史修正分支将远期历史转化为近期状态条件下的预测修正。这一近期—远期职责分化架构扩展了模型可访问的历史范围，同时将高成本的图传播和状态空间扫描集中于近期窗口。统一基线比较、多随机种子对照、历史干预和跨主干迁移共同表明，TimeRole 能够稳定利用样本匹配的远期历史，并以接近近期预测器的成本获得有竞争力的预测性能。因此，TimeRole 为多变量长期预测提供了一种兼顾历史利用与计算效率的职责分化建模范式。
