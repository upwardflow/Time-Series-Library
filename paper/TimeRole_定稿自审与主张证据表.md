...
| 主张 | 直接证据 | 支持状态 | 边界 |
|---|---|---|---|
| 完整模型在统一协议下具有总体竞争力 | 五数据集×四跨度的三随机种子测试 MSE/MAE，与7个本地基线比较；MSE为11/20最优、平均排名1.95，MAE为11/20最优、平均排名1.75 | 支持 | 相对各任务最佳基线，宏平均 MSE 高0.376%、MAE低0.059%；总体优势较小，不应表述为全面领先 |
...
| DHC 可迁移至非图主干 | TimeXer-336对照中4/4任务的三种子平均MSE/MAE均改善；种子级胜场为MSE 10/12、MAE 8/12 | 支持（已测范围） | 单额外主干、两个ETTm数据集、96/720两个跨度；不外推为任意主干通用 |
